...
/**
 * Runtime-representation for enum types. This is (in contrast to {@link AnyIntType} or
...
public class TypeEnum implements Type {
    ...
    public TypeEnum(String enumTypeName, List<String> values) {
        if (values.isEmpty()) {
            throw new IllegalArgumentException(
                    "Cannot create enum \"" + enumTypeName
                            + "\" without any values.");
        }
    ...
    @Override public String getSMTType() {
        return String.format("(_ BitVec %d)", this.valueList.size());
    }
    ...
    public ValueEnum valueOf(String enumName) {
        ...
        if (enumVal == null) {
            throw new RuntimeException(
                    "No enum value \"" + enumName + "\" for enum type: "
                            + this);
        }
        else {
            return enumVal;
