...
public class SmtEncoder {
    ...
    public SmtEncoder(List<Integer> maxDurations,
        ...
        if (maxDurations.size() != specification.getRows().size()) {
            throw new IllegalArgumentException(
                    "Size of maxDurations and size of specification rows do not accept");
        }
        ...
        for (ValidIoVariable ioVariable : this.ioVariables) {
            ...
            for (int z = 0; z < column.getCells().size(); z++) {
                ...
                this.constraint.addHeaderDefinitions(
                        new SList("declare-const", "n_" + z, "(_ BitVec 16)"));
    ...
    private static SList getDefaultValueEquality(ValidFreeVariable variable) {
        ...
        ValueLambdaVisitor<SList> visitor = new ValueLambdaVisitor<>();
        visitor.setValueIntFunction(
                (integerVal) -> new SList("=", "|" + variable.getName() + "|",
                        BitvectorUtils.hexFromInt(integerVal.getValue().intValue(),4)));

        visitor.setValueBoolFunction(
                boolVal -> new SList("=", "|" + variable.getName() + "|",
                        boolVal.getValue() ? "true" : "false"));

        visitor.setValueEnumFunction(
                enumVal -> new SList("=", "|" + variable.getName() + "|",
                        BitvectorUtils.hexFromInt(enumVal.getType().getValues()
                                .indexOf(enumVal),4)));

        return defaultValue.accept(visitor);
    }

    private static SList getDeclarationForVariable(Type type,
            String variableName) {
        return new SList("declare-const", "|" + variableName + "|",
                getSmtLibVariableTypeName(type));
    }

    protected static String getSmtLibVariableTypeName(Type type) {
        return type.getSMTType();
    }
