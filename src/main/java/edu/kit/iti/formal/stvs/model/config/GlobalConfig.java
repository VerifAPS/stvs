...
public class GlobalConfig {
    ...
    public GlobalConfig() {
        ...
        getetaCommand = new SimpleStringProperty(
                "java -jar /path/to/geteta.jar -c ${code} -t ${spec} -x");
    }
    ...
    public static GlobalConfig autoloadConfig() {
        File configFile = new File(
                CONFIG_DIRPATH + File.separator + AUTOLOAD_CONFIG_FILENAME);
        try {
            return ImporterFacade
                    .importConfig(configFile, ImporterFacade.ImportFormat.XML);
        }
    ...
    public void autosaveConfig() throws IOException, ExportException {
        ...
        if (!configDir.isDirectory() || !configDir.exists()) {
            ...
            if (!successful) {
                throw new IOException("The configuration directory " + configDir
                        .getAbsolutePath() + " could not be created.");
            }
        }
        File configFile = new File(
                CONFIG_DIRPATH + File.separator + AUTOLOAD_CONFIG_FILENAME);
        ExporterFacade.exportConfig(this, ExporterFacade.ExportFormat.XML,
                configFile);
    }
    ...
    public List<String> getProblems() {
        List<String> p = new ArrayList<>(5);
        File nuxmv = new File(getNuxmvFilename());
        if (!nuxmv.exists()) {
            p.add("Path to nuXmv does not exists or is not given.");
        }

        String getetaPath = getGetetaCommand();
        if (getetaPath.isEmpty())
            p.add("geteta executable is not set.");

        File g = new File(getetaPath.substring(0, getetaPath.indexOf(' ')));
        if (!g.exists()) {
            p.add("geteta " + g.getAbsolutePath()
                    + "executable does not exists.");
        }

        File z3 = new File(getZ3Path());
        if (!z3.exists()) {
            p.add("z3 executable is not valid.");
        }
        return p;
    }
    ...
    public void setVerificationTimeout(int verificationTimeout) {
        if (verificationTimeout <= 0) {
            throw new IllegalArgumentException(
                    "Invalid verification timeout: " + verificationTimeout);
        }
    ...
    public void setSimulationTimeout(int simulationTimeout) {
        if (simulationTimeout <= 0) {
            throw new IllegalArgumentException(
                    "Invalid simulation timeout: " + simulationTimeout);
        }
    ...
    public void setEditorFontSize(int editorFontSize) {
        if (editorFontSize <= 0) {
            throw new IllegalArgumentException(
                    "Invalid editor font size: " + editorFontSize);
        }
    ...
    public void setUiLanguage(String uiLanguage) {
        if (!validLanguages.contains(uiLanguage)) {
            throw new IllegalArgumentException(
                    "Input language " + uiLanguage + " is not supported");
        }
    ...
    public void setWindowHeight(int windowHeight) {
        if (windowHeight <= 0) {
            throw new IllegalArgumentException(
                    "Illegal window height: " + windowHeight);
        }
    ...
    public void setWindowWidth(int windowWidth) {
        if (windowWidth <= 0) {
            throw new IllegalArgumentException(
                    "Illegal window width: " + windowWidth);
        }
    ...
    @Override public boolean equals(Object obj) {
        ...
        if (getValidLanguages() != null ?
                !getValidLanguages().equals(that.getValidLanguages()) :
        ...
        if (getUiLanguage() != null ?
                !getUiLanguage().equals(that.getUiLanguage()) :
        ...
        if (getEditorFontFamily() != null ?
                !getEditorFontFamily().equals(that.getEditorFontFamily()) :
        ...
        if (getNuxmvFilename() != null ?
                !getNuxmvFilename().equals(that.getNuxmvFilename()) :
                that.getNuxmvFilename() != null) {
            return false;
        }
        if (getZ3Path() != null ?
                !getZ3Path().equals(that.getZ3Path()) :
                that.getZ3Path() != null) {
            return false;
        }
        return getGetetaCommand() != null ?
                getGetetaCommand().equals(that.getGetetaCommand()) :
                that.getGetetaCommand() == null;
    }

    @Override public int hashCode() {
        int result = getValidLanguages() != null ?
                getValidLanguages().hashCode() :
                0;
        ...
        result = 31 * result + (getUiLanguage() != null ?
                getUiLanguage().hashCode() :
                0);
        result = 31 * result + getMaxLineRollout();
        result = 31 * result + getEditorFontSize();
        result = 31 * result + (getEditorFontFamily() != null ?
                getEditorFontFamily().hashCode() :
                0);
        result = 31 * result + (isShowLineNumbers() ? 1 : 0);
        result = 31 * result + (getNuxmvFilename() != null ?
                getNuxmvFilename().hashCode() :
                0);
        result = 31 * result + (getZ3Path() != null ?
                getZ3Path().hashCode() :
                0);
        result = 31 * result + (getGetetaCommand() != null ?
                getGetetaCommand().hashCode() :
                0);
        return result;
    }
}
