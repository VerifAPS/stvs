 ...
 */
public class SmtConvertExpressionVisitor
        implements ExpressionVisitor<SExpression> {
    // static maps
    ...
    private static Map<BinaryFunctionExpr.Op, String> smtlibBinOperationNames = new HashMap<BinaryFunctionExpr.Op, String>() {
        {
     ...
     */
    public SmtConvertExpressionVisitor(SmtEncoder smtEncoder, int row,
            int iteration, ValidIoVariable column) {
        ...
        this.constraint = new SmtModel().addHeaderDefinitions(
                new SList("declare-const", name, SmtEncoder
                        .getSmtLibVariableTypeName(column.getValidType())));

        // Constraint enum bitvectors to their valid range
        if (column.getValidType() instanceof TypeEnum) {
            TypeEnum validType = (TypeEnum) column.getValidType();
            addEnumBitvectorConstraints(name, validType);
        }
    }
     ...
     */
    private void addEnumBitvectorConstraints(String name,
            TypeEnum enumeration) {
        this.constraint.addGlobalConstrains(
                new SList("bvsge", name, BitvectorUtils.hexFromInt(0,4)));
        this.constraint.addGlobalConstrains(new SList("bvslt", name,
                BitvectorUtils.hexFromInt(enumeration.getValues().size(),4)));
    }

    @Override public SExpression visitBinaryFunction(
            BinaryFunctionExpr binaryFunctionExpr) {
        SExpression left = binaryFunctionExpr.getFirstArgument()
                .takeVisitor(SmtConvertExpressionVisitor.this);
        SExpression right = binaryFunctionExpr.getSecondArgument()
                .takeVisitor(SmtConvertExpressionVisitor.this);
        ...
        default:
            String name = smtlibBinOperationNames
                    .get(binaryFunctionExpr.getOperation());
            if (name == null) {
                throw new IllegalArgumentException(
                        "Operation " + binaryFunctionExpr.getOperation()
                                + " is " + "not supported");
            }
    ...
    @Override public SExpression visitUnaryFunction(
            UnaryFunctionExpr unaryFunctionExpr) {
        SExpression argument = unaryFunctionExpr.getArgument()
                .takeVisitor(this);
        String name = smtlibUnaryOperationNames
                .get(unaryFunctionExpr.getOperation());

        if (name == null) {
            if (unaryFunctionExpr.getOperation()
                    == UnaryFunctionExpr.Op.UNARY_MINUS) {
                return new SList("-", new SAtom("0"), argument);
            }
            else {
                throw new IllegalArgumentException(
                        "Operation " + unaryFunctionExpr.getOperation() + "is "
                                + "not supported");
            }
        }
    ...
    @Override public SExpression visitLiteral(LiteralExpr literalExpr) {
        ValueLambdaVisitor<String> visitor = new ValueLambdaVisitor<>(
                integer -> integer.getValue().toString(16),
                this::getEnumValueAsBitvector,
                bool -> bool.getValue() ? "true" : "false");
        String literalAsString = literalExpr.getValue().accept(visitor);
        return new SAtom(literalAsString);
    }

    private String getEnumValueAsBitvector(ValueEnum enumeration) {
        return BitvectorUtils.hexFromInt(
                enumeration.getType().getValues().indexOf(enumeration),4);
    }

    @Override public SExpression visitVariable(VariableExpr variableExpr) {
        ...
        if (smtEncoder.getTypeForVariable(variableName) == null) {
            throw new IllegalStateException(
                    "Wrong Context: No variable of name '" + variableName
                            + "' in getTypeForVariable");
        }
            ...
            if (variableReferenceIndex < 0) {
                constraint.addGlobalConstrains(
                        ...
                        new SList("bvuge", sumRowIterations(row - 1), new SAtom(
                                BitvectorUtils.hexFromInt(Math.max(0,
                                        -(iteration + variableReferenceIndex)),4))));
            }
