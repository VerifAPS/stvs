...
public class StvsRootController implements Controller {
    ...
    public StvsRootController(StvsRootModel rootModel) {
        ...
        this.types = new SimpleObjectProperty<>(typesFromCode(
                stvsRootModel.getScenario().getCode().getParsedCode()));
        this.ioVars = new SimpleObjectProperty<>(ioVarsFromCode(
                stvsRootModel.getScenario().getCode().getParsedCode()));
        this.specificationsPaneController = new SpecificationsPaneController(
                stvsRootModel.getHybridSpecifications(),
                stvsRootModel.getScenario().verificationState(), types, ioVars,
                stvsRootModel.getGlobalConfig(), stvsRootModel.getScenario());

        this.stvsRootModel.getScenario().codeObjectProperty()
                .addListener(this::onCodeChange);
        this.stvsRootModel.getScenario().getCode().parsedCodeProperty()
                .addListener((obs, old, parsedCode) -> onParsedCodeChange(
                        parsedCode));
        this.stvsRootModel.getScenario().verificationResultProperty()
                .addListener(this::onVerificationResultChange);

        this.view = new StvsRootView(editorPaneController.getView(),
        ...
        getView().getNotificationPane().show("dskfjdskfjsfjsdf");
        displayConfigurationProblems();
    }

    private void displayConfigurationProblems() {
        List<String> problems = stvsRootModel.getGlobalConfig().getProblems();
        if (problems.size() != 0) {
            view.getNotificationPane()
                    .show(problems.stream().reduce((s, t) -> s + "\n" + t)
                            .orElse(""));
            view.getNotificationPane().getActions()
                    .addAll(new Action("Open Preferences", (event) -> {
                        //TODO open preferences dialog
                        ConfigDialogManager cfm = new ConfigDialogManager(
                                getRootModel().getGlobalConfig());
                        cfm.showAndWait();
                        view.getNotificationPane().hide();
                    }));
            view.getNotificationPane().show();
        }
    }
    ...
    private void onVerificationEvent(VerificationEvent event) {
        ...
        default:
            throw new IllegalStateException(
                    "Could not handle verification event type.");
        }
    }
     ...
     */
    private void onCodeChange(ObservableValue<? extends Code> observableValue,
            Code old, Code code) {
        editorPaneController = new EditorPaneController(code,
                stvsRootModel.getGlobalConfig());
     ...
     */
    private void onVerificationResultChange(
            ObservableValue<? extends VerificationResult> o,
