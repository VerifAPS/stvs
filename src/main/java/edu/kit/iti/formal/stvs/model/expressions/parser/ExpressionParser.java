...
public class ExpressionParser extends CellExpressionBaseVisitor<Expression> {
    ...
    private Map<String, ValueEnum> computeEnumValuesByName(
            Collection<Type> typeSet) {
        Map<String, ValueEnum> byName = new HashMap<>();
        typeSet.stream().map(this::filterEnumType).filter(Optional::isPresent)
                .map(Optional::get) // Filter
                .forEach(typeEnum -> typeEnum.getValues()
                        .forEach(valueEnum -> // For every possible enum value
                                        byName.put(valueEnum.getEnumValue(), valueEnum)
                                // sort it in by name
                        ));
        return byName;
    }
    ...
    private Optional<TypeEnum> filterEnumType(Type type) {
        if (type instanceof TypeEnum) {
            return Optional.of((TypeEnum) type);
        }
    ...
    @Override public Expression visit(ParseTree tree) {
        return tree.accept(this);
    }

    @Override public Expression visitCell(
            CellExpressionParser.CellContext ctx) {
        Optional<Expression> optionalExpression = ctx.chunk().stream()
                .map(chunkContext -> chunkContext.accept(this))
                .reduce((e1, e2) -> new BinaryFunctionExpr(
                        BinaryFunctionExpr.Op.AND, e1, e2));
    ...
    @Override public Expression visitDontcare(
            CellExpressionParser.DontcareContext ctx) {
        return new LiteralExpr(ValueBool.TRUE);
    }

    @Override public Expression visitConstant(
            CellExpressionParser.ConstantContext ctx) {
        Expression literalExpr = new LiteralExpr(valueFromConstantToken(ctx));
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.EQUALS,
                columnAsVariable, literalExpr);
    }

    @Override public Expression visitBconstant(
            CellExpressionParser.BconstantContext ctx) {
        return new LiteralExpr(valueFromConstantToken(ctx.constant()));
    }

    @Override public Expression visitVariable(
            CellExpressionParser.VariableContext ctx) {
        ...
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.EQUALS,
                columnAsVariable, variableExpr);
    }

    @Override public Expression visitBvariable(
            CellExpressionParser.BvariableContext ctx) {
    ...
    private Expression parseOccuringString(
            CellExpressionParser.VariableContext ctx) {
        return parseArrayIndex(ctx).map(index ->
                ...
                (Expression) new VariableExpr(parseIdentifier(ctx),
                        index)) // really java? really?
                // Otherwise we still have to find out
                .orElse(maybeParseEnum(ctx));
    }

    private Expression maybeParseEnum(
            CellExpressionParser.VariableContext ctx) {
    ...
    private Optional<Integer> parseArrayIndex(
            CellExpressionParser.VariableContext ctx) {
    ...
    private static Integer getArrayIndex(TerminalNode node) {
        ...
        if (index > 0) {
            throw new UnsupportedExpressionRuntimeException(
                    "Positive Variable Index");
        }
        return index;
    }
    ...
    private Value valueFromConstantToken(
            CellExpressionParser.ConstantContext ctx) {
    ...
    @Override public Expression visitSinglesided(
            CellExpressionParser.SinglesidedContext ctx) {
    ...
    @Override public Expression visitPlus(
            CellExpressionParser.PlusContext ctx) {
    ...
    @Override public Expression visitSubstract(
            CellExpressionParser.SubstractContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.MINUS, left, right);
    }

    @Override public Expression visitMult(
            CellExpressionParser.MultContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.MULTIPLICATION,
                left, right);
    }

    @Override public Expression visitDiv(CellExpressionParser.DivContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.DIVISION, left,
                right);
    }

    @Override public Expression visitMod(CellExpressionParser.ModContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.MODULO, left,
                right);
    }

    @Override public Expression visitLogicalAnd(
            CellExpressionParser.LogicalAndContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.AND, left, right);
    }

    @Override public Expression visitLogicalXor(
            CellExpressionParser.LogicalXorContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.XOR, left, right);
    }

    @Override public Expression visitLogicalOr(
            CellExpressionParser.LogicalOrContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.OR, left, right);
    }

    @Override public Expression visitInequality(
            CellExpressionParser.InequalityContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.NOT_EQUALS, left,
                right);
    }

    @Override public Expression visitEquality(
            CellExpressionParser.EqualityContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.EQUALS, left,
                right);
    }

    @Override public Expression visitCompare(
            CellExpressionParser.CompareContext ctx) {
        Expression left = ctx.left.accept(this);
        Expression right = ctx.right.accept(this);
        return new BinaryFunctionExpr(binaryOperationFromToken(ctx.op), left,
                right);
    }

    @Override public Expression visitInterval(
            CellExpressionParser.IntervalContext ctx) {
    ...
    private Expression makeInterval(Expression variable, Expression lower,
            Expression upper) {
        ...
        return new BinaryFunctionExpr(BinaryFunctionExpr.Op.AND,
                greaterThanLower, smallerThanUpper);
    }

    @Override public Expression visitMinus(
            CellExpressionParser.MinusContext ctx) {
        Expression toBeNegated = ctx.sub.accept(this);
        return new UnaryFunctionExpr(UnaryFunctionExpr.Op.UNARY_MINUS,
                toBeNegated);
    }

    @Override public Expression visitNegation(
            CellExpressionParser.NegationContext ctx) {
        return new UnaryFunctionExpr(UnaryFunctionExpr.Op.NOT,
                ctx.sub.accept(this));
    }

    @Override public Expression visitParens(
            CellExpressionParser.ParensContext ctx) {
        return ctx.sub.accept(this);
    }

    @Override public Expression visitGuardedcommand(
            CellExpressionParser.GuardedcommandContext ctx) {
        throw new UnsupportedExpressionRuntimeException("Guarded command (if)");
    }

    @Override public Expression visitFunctioncall(
            CellExpressionParser.FunctioncallContext ctx) {
